...
const desencriptarMensaje = (mensajeEncriptado: any, clave: any) => {
  ...
  const mensajeBytes = mensajeEncriptado.match(/.{2}/g).map(byte => parseInt(byte, 16));

  // Aplicar la S-Box inversa
  const mensajeSBoxInversa = mensajeBytes.map(byte => {
    const fila = byte >> 4;
  ...
  // XOR con la clave
  const claveBytes = clave.split("").map(char => char.charCodeAt(0));
  const mensajeOriginal = mensajeSBoxInversa.map((byte, index) => byte ^ claveBytes[index]);
...
export const PaginaMensajes: React.FC<PaginaMensajesProps> = ({
  ...
  const fetchMensajes = async (sender: string, receiver: string) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Error al recuperar los mensajes, código: ${response.status}`);
      }
      ...
      if (!data || data.length === 0) {
        throw new Error("La respuesta JSON está vacía o mal formada.");
      }
      ...
      return [];  // Maneja correctamente el caso de error
    }
      ...
      if (idUsuario) {
        Promise.all([
          ...
          .then(([mensajesEnviados, mensajesRecibidos]) => {
            const todosLosMensajes = [...mensajesEnviados, ...mensajesRecibidos].sort(
              (a, b) => new Date(a.fecha + " " + a.hora).getTime() - new Date(b.fecha + " " + b.hora).getTime()
            );
        ...
        <form
          ...
          onSubmit={async (e) => {
            ...
            try {
              ...
              // Debugging: Verificar sender y receiver
              ...
              const response = await fetch(`http://127.0.0.1:8000/api/messages/`, {
                ...
                body: JSON.stringify({
                  sender: sender,
                  content: mensaje, // Envía el mensaje como está
                  message_type: "text",
                  status: "sent",
                  is_deleted: 0,
                  media_url: null,
              ...
              const nuevosMensajes = await fetchMensajes(sender, userContacto);
